...
func (tCertPool *tCertPoolSingleThreadImpl) Start() (err error) {
	...
	if len(tCertDERs) == 0 {
		...
		//tCertPool.client.gtoGetTCertsFromTCA(tCertPool.client.conf.getTCertBatchSize()/2, "query")
		//tCertPool.client.gtoGetTCertsFromTCA(tCertPool.client.conf.getTCertBatchSize()/2, "invoke")
	} else {
		tCertPool.client.debug("TCerts in cache found! Loading them...")

		for _, tCertDER := range tCertDERs {
			tCertPool.client.debug("calling getTCertFromDER...")
...
func (tCertPool *tCertPoolSingleThreadImpl) GtoGetNextTCert(methodName string) (tCert tCert, err error) {

	var l sync.Mutex
	l.Lock()

	tCertPool.client.info("Single Thread GetNextTCert()1")
	//tCertPool.m.Lock()
	//defer tCertPool.m.Unlock()

	if tCertPool.len <= 0 {
		// Reload
		tCertPool.client.info("Single Thread GetNextTCert()2")
		if err := tCertPool.client.gtoGetTCertsFromTCA(tCertPool.client.conf.getTCertBatchSize(), methodName); err != nil {

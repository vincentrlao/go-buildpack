...
func (client *clientImpl) getTCertFromDER(der []byte) (tCert tCert, err error) {
	...
	TCertIndex := tCertIndexCT //I didnt encrypt the TCERTID this time (SYDNEY)
	//TCertIndex := pt
...
func (client *clientImpl) gtoGetTCertsFromTCA(num int, methodName string) error {
	client.info("Aung getTCertsFromTCA %d", num)
	client.debug("Get [%d] certificates from the TCA...", num)

	// Contact the TCA
	TCertOwnerKDFKey, certDERs, err := client.gtoCallTCACreateCertificateSet(num, methodName) //callTCACreateCertificateSet(num)
	if err != nil {
	...
	for i := 0; i < num; i++ {
		...
		TCertIndex := tCertIndexCT // im sending it in plain (SYDNEY)
		// Compute ExpansionValue based on TCertIndex
		//TCertIndex := pt
		//		TCertIndex := []byte(strconv.Itoa(i))
...
func (client *clientImpl) gtoCallTCACreateCertificateSet(num int, methodName string) ([]byte, []*membersrvc.TCert, error) {
	// Get a TCA Client
	...
	if client.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		enableLunaHSM = true
	}
	...
	var rawReq []byte
	var gtoReq *membersrvc.GTOTCertCreateSetReq

	if enableLunaHSM {

		client.info("gtoCallTCACreateCertificateSet call to GenerateECDSAKeypair")

		rawHSMPubKey, err := hsm.GenerateECDSAKeypair(client.enrollID+"_"+methodName+"_tx", client.conf.securityLevel)
		if err != nil {
			client.error("Failed to generate ECDSA keypair for transaction [%s].", err.Error())
			return nil, nil, err
		}

		ecdsaPubKey, err := hsm.ECDSARawtoPubKey(rawHSMPubKey, client.conf.securityLevel)
		if err != nil {
			client.error("Failed converting ECDSA raw public key to golang ecdsa public key type [%s].", err.Error())
			return nil, nil, err
		}

		derPubKey, err := x509.MarshalPKIXPublicKey(ecdsaPubKey)
		if err != nil {
			client.error("Failed mashalling ECDSA key [%s].", err.Error())
			return nil, nil, err
		}

		gtoReq = &membersrvc.GTOTCertCreateSetReq{
			Ts:     &timestamp,
			Tcerts: req,
			PubKey: &membersrvc.PublicKey{Type: membersrvc.CryptoType_ECDSA, Key: derPubKey},
		}

		rawReq, err = proto.Marshal(req) //Peer-Wallet_TODO: Signature must be for GTOTCertRequest
	} else {
	...
	if enableLunaHSM {
		...
		signature, err := hsm.ECDSASign(client.enrollID+"_enroll", hash.Sum(nil))
		if err != nil {
		...
		if err != nil {
			client.error("Failed parsing ECDSA signature to R and S [%s].", err.Error())
			return nil, nil, err
		}
	} else {
		//Hyperledger way
	...
	// 4. Send request
	var certSet *membersrvc.TCertCreateSetResp

	client.error("Reaching here 1")

	if enableLunaHSM {

		client.error("Reaching here 1.1")
		gtoReq.Tcerts.Sig = &membersrvc.Signature{Type: membersrvc.CryptoType_ECDSA, R: R, S: S}
		certSet, err = tcaP.GTOCreateCertificateSet(context.Background(), gtoReq)
	} else {
...
func (client *clientImpl) callTCACreateCertificateSet(num int) ([]byte, []*membersrvc.TCert, error) {
	...
	if enableLunaHSM {
		rawHSMPubKey, err := hsm.GenerateECDSAKeypair(client.enrollID+"_tx", client.conf.securityLevel)
		if err != nil {
			client.error("Failed to generate ECDSA keypair for transaction [%s].", err.Error())
			return nil, nil, err
		}

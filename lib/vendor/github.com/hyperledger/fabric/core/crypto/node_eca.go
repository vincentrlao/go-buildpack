...
func (node *nodeImpl) getEnrollmentCertificateFromECA(id, pw string) (interface{}, []byte, []byte, error) {
	...
	//Peer-Wallet_TODO: To generate Enrollment ECDSA keypair on Luna HSM or using hyperledger way
	enableLunaHSM := false
	if node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		enableLunaHSM = true
	}

	//Peer-Wallet_TODO: To enable using LunaSA HSM for Enrollment, comment the following line
	//enableLunaHSM = false

	var signPriv *ecdsa.PrivateKey
	var derSignPubKey []byte

	if enableLunaHSM {
		//Generating ECDSA keypair on Luna HSM
		rawHSMPubKey, err := hsm.GenerateECDSAKeypair(id + "_enroll", node.conf.securityLevel)
		if err != nil {
			node.error("Failed generating ECDSA key on Luna HSM [%s].", err.Error())
			return nil, nil, nil, err
		}

		ecdsaPubKey, err := hsm.ECDSARawtoPubKey(rawHSMPubKey, node.conf.securityLevel)
		if err != nil {
			node.error("Failed converting ECDSA raw public key to golang ecdsa public key type [%s].", err.Error())
			return nil, nil, nil, err
		}

		//Dummy Private Key Value (to avoid nil error during storePrivateKey)
		var D big.Int
		signPriv = &ecdsa.PrivateKey{ecdsa.PublicKey(*ecdsaPubKey),&D,}

		derSignPubKey, err = x509.MarshalPKIXPublicKey(ecdsaPubKey)
		if err != nil {
			node.error("Failed mashalling ECDSA key [%s].", err.Error())
			return nil, nil, nil, err
		}
	} else {
		//Hyperledger original way
		...
		derSignPubKey, err = x509.MarshalPKIXPublicKey(&signPriv.PublicKey)
		if err != nil {
			node.error("Failed mashalling ECDSA key [%s].", err.Error())
	...
	// Generating ECDSA keypair for encryption and decrytpion on local
	...
	node.error("Enroll Secret Token %s", pw)
	...
	req := &membersrvc.ECertCreateReq{
		...
		Sign: &membersrvc.PublicKey{Type: membersrvc.CryptoType_ECDSA, Key: derSignPubKey},
	...
	//Peer-Wallet_TODO: To sign ECertRequest with ECDSA private key from Luna HSM
	var r, s *big.Int

	if enableLunaHSM {
		//Using luna HSM
		signature, err := hsm.ECDSASign(id + "_enroll", hash.Sum(nil))
		if err != nil {
			node.error("Failed signing with Luna HSM ECDSA private key [%s].", err.Error())
			return nil, nil, nil, err
		}

		r, s, err = hsm.ECDSASigtoRS(signature)
		if err != nil {
			node.error("Failed parsing ECDSA signature to R and S [%s].", err.Error())
			return nil, nil, nil, err
		}

...
var (
	cryptoLogger = logging.MustGetLogger("crypto")
)
...
func (ks *keyStore) storePrivateKey(alias string, privateKey interface{}) error {
	cryptoLogger.Info("storePrivateKey")
	...
	/*
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		err = hsm.StorePrivateKey(keyID, rawKey)
		if err != nil {
			ks.node.error("Failed storing private key on HSM [%s]: [%s]", keyID, err)
			return err
		}
	} else {
	*/
...
func (ks *keyStore) storePrivateKeyInClear(alias string, privateKey interface{}) error {
	cryptoLogger.Info("storePrivateKeyInClear")
...
func (ks *keyStore) loadPrivateKey(alias string) (privateKey interface{}, err error) {
	cryptoLogger.Info("loadPrivateKey")
	...
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		ks.node.info("Loading private key from HSM (%s)", keyID)

		raw, err = hsm.LoadPrivateKey(keyID)
		if err != nil {
			ks.node.error("Failed loading private key from HSM [%s]: [%s].", keyID, err.Error())
...
func (ks *keyStore) storePublicKey(alias string, publicKey interface{}) (err error) {
	cryptoLogger.Info("storePublicKey")
	...
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		err = hsm.StorePublicKey(keyID, rawKey)
		if err != nil {
			ks.node.error("Failed storing public key on HSM [%s]: [%s]", keyID, err)
			return err
		}
	} else {
	*/
		err = ioutil.WriteFile(ks.node.conf.getPathForAlias(alias), rawKey, 0700)
		if err != nil {
			ks.node.error("Failed storing public key [%s]: [%s]", alias, err)
			return err
		}
	//}
...
func (ks *keyStore) loadPublicKey(alias string) (publicKey interface{}, err error) {
	cryptoLogger.Info("loadPublicKey")
	...
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		ks.node.info("Loading public key from HSM (%s)", keyID)

		raw, err = hsm.LoadPublicKey(keyID)
		if err != nil {
			ks.node.error("Failed loading public key from HSM [%s]: [%s].", keyID, err.Error())
	...
	*/
	...
	if err != nil {
		ks.node.error("Failed parsing public key [%s]: [%s].", alias, err.Error())
...
func (ks *keyStore) storeKey(alias string, key []byte) (err error) {
	cryptoLogger.Info("storeKey")
	...
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		err = hsm.StoreKey(keyID, pem)
		if err != nil {
			ks.node.error("Failed storing key [%s]: [%s]", keyID, err)
			return err
		}
	} else {
	*/
...
func (ks *keyStore) loadKey(alias string) (key []byte, err error) {
	cryptoLogger.Info("loadKey")
	...
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		keyID := ks.node.conf.name + "_" + alias
		ks.node.info("Loading key from HSM (%s)", keyID)

		pem, err = hsm.LoadKey(keyID)
		if err != nil {
			ks.node.error("Failed loading key from HSM [%s]: [%s].", keyID, err.Error())
...
func (ks *keyStore) storeCert(alias string, der []byte) (err error) {
	cryptoLogger.Info("storeCert")

	/*
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		certID := ks.node.conf.name + "_" + alias
		err = hsm.StoreCert(certID, utils.DERCertToPEM(der))
		if err != nil {
			ks.node.error("Failed storing certificate [%s]: [%s]", certID, err)
			return err
		}
	} else {
	*/
...
func (ks *keyStore) loadCert(alias string) (pem []byte, err error) {
	cryptoLogger.Info("loadCert")

	/*
	if ks.node.eType == NodeClient && viper.GetBool("peer.wallet.enabled") {
		certID := ks.node.conf.name + "_" + alias
		ks.node.info("Loading certificate from HSM (%s)", certID)

		pem, err = hsm.LoadCert(certID)
		if err != nil {
			ks.node.error("Failed loading certificate from HSM [%s]: [%s].", certID, err.Error())
...
func (ks *keyStore) loadExternalCert(path string) ([]byte, error) {
	cryptoLogger.Info("loadExternalCert")
...
func (ks *keyStore) loadCertX509AndDer(alias string) (*x509.Certificate, []byte, error) {
	cryptoLogger.Info("loadCertX509AndDer")
...
func (ks *keyStore) createKeyStoreIfNotExists() error {
	cryptoLogger.Info("createKeyStoreIfNotExists")
...
func (ks *keyStore) createKeyStore() error {
	cryptoLogger.Info("createKeyStore")
...
func (ks *keyStore) deleteKeyStore() error {
	cryptoLogger.Info("deleteKeyStore")
...
func (ks *keyStore) openKeyStore() error {
	cryptoLogger.Info("openKeyStore")

...
func (s *ServerOpenchainREST) CreateWallet(rw web.ResponseWriter, req *web.Request) {
	restLogger.Info("REST Create Wallet...")

	// Checking Peer Wallet Enabled or Not
	if viper.GetBool("peer.wallet.enabled") == false {
		rw.WriteHeader(http.StatusForbidden)
		fmt.Fprintf(rw, "{\"Error\": \"This peer is not a peer-wallet.\"}")
		restLogger.Error("{\"Error\": \"This peer is not a peer-wallet.\"}")
	...
	restLogger.Info("Local data store for client wallet credential: %s", localStore)

	// If the user has wallet already, return
	if _, err := os.Stat(localStore + "wallet_" + loginSpec.EnrollId); err == nil {
		rw.WriteHeader(http.StatusOK)
		fmt.Fprintf(rw, "{\"OK\": \"User %s has wallet already.\"}", loginSpec.EnrollId)
		restLogger.Info("User %s has wallet already.\n", loginSpec.EnrollId)

		return
	}

	// User does not have wallet, proceed
	restLogger.Info("Creating wallet credential for user '%s' on REST interface...\n", loginSpec.EnrollId)

	nullByte := []byte{0x00}
	salt := []byte("nUuasfB92eShdt0L") // hard-coded salt temporarily for testing
	partition_pwd := append([]byte("S@fenet123"), nullByte[:]...)

	cipherText := toolbox.Pbkdf2Encrypt([]byte(loginSpec.EnrollSecret), salt, partition_pwd)

	// if cipherText is not nil
	if cipherText != nil {
		...
		// Store client wallet credential into a file
		restLogger.Info("Storing wallet credential for user '%s'.\n", loginSpec.EnrollId)
		err = ioutil.WriteFile(localStore+"wallet_"+loginSpec.EnrollId, cipherText, 0755)
		if err != nil {
			rw.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(rw, "{\"Error\": \"Fatal error -- %s\"}", err)
			panic(fmt.Errorf("Fatal error when storing client wallet credential: %s\n", err))
		}

		rw.WriteHeader(http.StatusOK)
		fmt.Fprintf(rw, "{\"OK\": \"Creating wallet credential successful for user '%s'.\"}", loginSpec.EnrollId)
		restLogger.Info("Creating wallet credential successful for user '%s'.\n", loginSpec.EnrollId)
	} else {
		loginErr := "Failed to create wallet credential."

		rw.WriteHeader(http.StatusUnauthorized)
		fmt.Fprintf(rw, "{\"Error\": \"%s\"}", loginErr)
		restLogger.Error(fmt.Sprintf("Error on client wallet credential creation: %s", loginErr))
	}
...
func (s *ServerOpenchainREST) Register(rw web.ResponseWriter, req *web.Request) {
	...
	if viper.GetBool("peer.wallet.enabled") {
		// Decrypt HSM Partition Password
		salt := []byte("nUuasfB92eShdt0L") // hard-coded salt temporarily for testing
		password := []byte(loginSpec.EnrollSecret)

		// If the user does not have wallet credential, return
		if _, err := os.Stat(localStore + "wallet_" + loginSpec.EnrollId); err != nil {
			rw.WriteHeader(http.StatusOK)
			fmt.Fprintf(rw, "{\"OK\": \"User %s doesn't have wallet credential.\"}", loginSpec.EnrollId)
			restLogger.Info("User %s doesn't have wallet credential.\n", loginSpec.EnrollId)

			return
		}

		cipherLunaPwd := toolbox.ReadFile(localStore + "wallet_" + loginSpec.EnrollId)

		// Decrypt HSM Partition Password
		decryptedPwd := toolbox.Pbkdf2Decrypt(password, salt, cipherLunaPwd)
		nullPos := bytes.IndexByte(decryptedPwd, 0)

		var pwd string
		if nullPos >= 0 && nullPos < len(decryptedPwd) {
			pwd = string(decryptedPwd[:nullPos])
		} else {
			pwd = string(decryptedPwd[:])
		}

		restLogger.Info("Enroll Secret is " + loginSpec.EnrollSecret)
		restLogger.Info("Partition Password is " + pwd)

		// If there is active session, Uninitialize and close session
		if hsm.IsSessionActive() {
			restLogger.Info("Uninitlaize and Close Existing All HSM Sessions")
			hsm.CloseSession()
		}

		err = hsm.OpenSession(loginSpec.EnrollId, pwd)
		if err != nil {
			rw.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(rw, "{\"Error\": \"Fatal error -- %s\"}", err)
			restLogger.Error("{\"Error\": \"Fatal error -- %s\"}", err)
...
func (s *ServerOpenchainREST) GetEnrollmentCert(rw web.ResponseWriter, req *web.Request) {
	...
	if core.SecurityEnabled() {
		...
		restLogger.Debug("rest_api.go: GetEnrollmentCert CloseClient")
...
func (s *ServerOpenchainREST) processChaincodeInvokeOrQuery(method string, spec *pb.ChaincodeInvocationSpec) rpcResult {
	...
	if method == "query" {
		...
		restLogger.Error(fmt.Sprintf("Successfuly queried chaincode: %s", val))
	}
...
func StartOpenchainRESTServer(server *ServerOpenchain, devops *core.Devops) {
	...
	router.Post("createwallet", (*ServerOpenchainREST).CreateWallet)

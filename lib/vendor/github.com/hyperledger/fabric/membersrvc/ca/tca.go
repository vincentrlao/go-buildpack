...
// CUSTOM Gemalto
func (tcap *TCAP) GTOCreateCertificateSet(ctx context.Context, in *pb.GTOTCertCreateSetReq) (*pb.TCertCreateSetResp, error) {

	Trace.Println("grpc TCAP:GTOCreateCertificateSet ")
	id := in.Tcerts.Id.Id
	userPub := in.PubKey.Key
	...
	Trace.Println("SURVIVED 1")

	enrollCert, err := x509.ParseCertificate(raw)
	if err != nil {
		return nil, err
	}

	Trace.Println("SURVIVED 2")

	enrollPub := enrollCert.PublicKey.(*ecdsa.PublicKey)

	pubTmp, err := x509.ParsePKIXPublicKey(userPub)
	if err != nil {
		return nil, err
	}

	Trace.Println("SURVIVED 3")

	pub := pubTmp.(*ecdsa.PublicKey)

	r, s := big.NewInt(0), big.NewInt(0)
	r.UnmarshalText(in.Tcerts.Sig.R)
	s.UnmarshalText(in.Tcerts.Sig.S)

	//sig := in.Sig
	in.Tcerts.Sig = nil

	hash := primitives.NewHash()
	raw, _ = proto.Marshal(in.Tcerts)
	hash.Write(raw)
	if ecdsa.Verify(enrollPub, hash.Sum(nil), r, s) == false {
	...
	for i := 0; i < num; i++ {
		...
		extensions, ks, err := tcap.generateExtensions(tcertid, tidx, enrollCert, in.Tcerts.Attributes)
		if err != nil {
			return nil, err
		}

		spec := NewDefaultPeriodCertificateSpec(id, tcertid, pub, x509.KeyUsageDigitalSignature, extensions...)
		if raw, err = tcap.tca.createCertificateFromSpec(spec, in.Ts.Seconds, nil); err != nil {
			Error.Println(err)
			return nil, err
		}
	...
	return &pb.TCertCreateSetResp{&pb.CertSet{in.Ts, in.Tcerts.Id, nil, set}}, nil
}
